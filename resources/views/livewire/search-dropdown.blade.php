<div
class="relative mt-3 md:mt-0" x-data="{ isOpen: true }"
@click.away="isOpen = false">
    <input
    wire:model.debouce.500ms="search" type="text"
            ...
            @if ($searchResult->count() > 0)
                <ul>
                    ...
                    <li class="border-b border-gray-700">
                        ...
                        <a
                        href="{{ route('movies.show',$result['id'])}}" class="block hover:bg-gray-700 px-3 py-3 flex item-center transition ease-in-out duration-150"
                        @if($loop->last) @keydown.tab="isOpen = false" @endif
                        >
                            @if ($result['poster_path'])
                            <img src={{env('TMDB_IMGURL').'w92'.$result['poster_path']}} alt="{{$result['original_title']}}" class="w-8" >
                            @else
